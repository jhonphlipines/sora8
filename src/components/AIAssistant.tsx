...
export const AIAssistant = ({ context, className }: AIAssistantProps) => {
  ...
  const [position, setPosition] = useState({ left: window.innerWidth - 400, top: window.innerHeight - 450 }); // Start at bottom-right
  ...
  useEffect(() => {
    ...
    if (savedPosition) {
      ...
      setPosition({ left: window.innerWidth - 400, top: window.innerHeight - 450 });
    }
  }, []);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const languageInstruction = selectedLanguage !== 'en' 
        ? `IMPORTANT: Respond in ${currentLanguage?.name} language. All explanations, examples, and text should be in ${currentLanguage?.name}.` 
        : '';
      
      const programmingContext = selectedProgrammingLanguage !== 'general' 
        ? `Focus specifically on ${currentProgrammingLang?.name} programming language. Provide examples, best practices, and explanations related to ${currentProgrammingLang?.name}.`
        : '';
    ...
    return (
      ...
      >
        <Button
          ...
          size="lg"
          className="rounded-full shadow-lg bg-[var(--gradient-primary)] border-0 h-14 w-14 hover:scale-110 transition-transform drag-handle"
        >
          <MessageCircle className="h-6 w-6" />
        </Button>
        <Badge 
          variant="secondary" 
          className="absolute -top-2 -left-2 bg-primary text-primary-foreground animate-pulse pointer-events-none"
        >
  ...
  return (
    ...
    >
      <Card className={cn(
        "w-80 shadow-2xl border-border/50 bg-card/95 backdrop-blur-sm transition-all duration-300",
                ...
                <div className="p-4 border-t border-border/50">
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-3">
                    <div>
                      <Select value={selectedLanguage} onValueChange={setSelectedLanguage}>
                        ...
                        <SelectContent className="bg-background border border-border z-50">
                          {languages.map((lang) => (
                            <SelectItem key={lang.code} value={lang.code} className="hover:bg-muted text-xs">
                              <div className="flex items-center gap-2">
                  ...
                  <div className="flex gap-2">
                    <Input
                      ref={inputRef}
                      placeholder={
                        selectedProgrammingLanguage !== 'general' 
                          ? `Ask about ${programmingLanguages.find(p => p.code === selectedProgrammingLanguage)?.name}...`
                          : selectedLanguage === 'en' ? "Ask me anything about programming..." : 
                            selectedLanguage === 'es' ? "Pregúntame sobre programación..." :
                            selectedLanguage === 'fr' ? "Demandez-moi sur la programmation..." :
                            selectedLanguage === 'de' ? "Fragen Sie mich über Programmierung..." :
                            selectedLanguage === 'it' ? "Chiedimi sulla programmazione..." :
                            selectedLanguage === 'pt' ? "Pergunte-me sobre programação..." :
                            selectedLanguage === 'ru' ? "Спросите меня о программировании..." :
                            selectedLanguage === 'zh' ? "问我编程问题..." :
                            selectedLanguage === 'ja' ? "プログラミングについて聞いてください..." :
                            selectedLanguage === 'ko' ? "프로그래밍에 대해 물어보세요..." :
                            selectedLanguage === 'ar' ? "اسألني عن البرمجة..." :
                            selectedLanguage === 'hi' ? "प्रोग्रामिंग के बारे में पूछें..." :
