...
const postSchema = new mongoose.Schema({
  ...
  comments: [{
    ...
    // JavaScript Advanced Course
    {
      ...
      slides: [
        { title: "Closures and Scope", content: "Closures are functions that retain access to variables from their outer scope even after the outer function has returned. They enable data privacy and function factories.", keyPoints: ["Closures preserve outer scope variables", "Enable private variables and encapsulation", "Foundation for module pattern", "Important for callbacks and event handlers"], codeExample: "function createCounter() {\n  let count = 0;\n  return {\n    increment: () => ++count,\n    decrement: () => --count,\n    getCount: () => count\n  };\n}\n\nconst counter = createCounter();\nconsole.log(counter.increment()); // 1", exercise: "Create a closure-based 'bankAccount' with deposit, withdraw, and getBalance methods." },
        { title: "Prototypes and Inheritance", content: "JavaScript uses prototypal inheritance where objects can inherit from other objects through the prototype chain.", keyPoints: ["Objects have hidden [[Prototype]] property", "Prototype chain for property lookup", "ES6 classes are syntactic sugar", "Object.create() for prototype inheritance"], codeExample: "class Vehicle {\n  constructor(make) { this.make = make; }\n  getInfo() { return this.make; }\n}\n\nclass Car extends Vehicle {\n  constructor(make, model) {\n    super(make);\n    this.model = model;\n  }\n}", exercise: "Create a prototype chain for Animal -> Dog -> Puppy." },
        { title: "Design Patterns", content: "Design patterns are proven solutions to common programming problems including Singleton, Factory, Observer, and Module patterns.", keyPoints: ["Singleton ensures single instance", "Factory creates objects dynamically", "Observer for event-driven programming", "Module for encapsulation"], codeExample: "// Observer Pattern\nclass EventEmitter {\n  constructor() { this.events = {}; }\n  on(event, cb) {\n    if (!this.events[event]) this.events[event] = [];\n    this.events[event].push(cb);\n  }\n  emit(event, data) {\n    this.events[event]?.forEach(cb => cb(data));\n  }\n}", exercise: "Implement a Pub/Sub system with subscribe, unsubscribe, and publish methods." },
        { title: "Generators and Iterators", content: "Generators are functions that can pause and resume execution, useful for lazy evaluation and async flow control.", keyPoints: ["function* syntax with yield", "Iterator protocol", "Lazy evaluation for efficiency", "Useful for infinite sequences"], codeExample: "function* fibonacci() {\n  let [prev, curr] = [0, 1];\n  while (true) {\n    yield curr;\n    [prev, curr] = [curr, prev + curr];\n  }\n}\n\nconst fib = fibonacci();\nconsole.log(fib.next().value); // 1", exercise: "Create a generator that produces paginated API results." },
        { title: "Proxies and Reflection", content: "Proxies allow intercepting and customizing operations on objects for validation, logging, and reactive programming.", keyPoints: ["Intercept get, set, delete operations", "Validation and transformation", "Reactive programming patterns", "Reflect API for default behavior"], codeExample: "const validator = {\n  set(target, prop, value) {\n    if (prop === 'age' && typeof value !== 'number') {\n      throw new TypeError('Age must be a number');\n    }\n    target[prop] = value;\n    return true;\n  }\n};\n\nconst person = new Proxy({}, validator);", exercise: "Create a proxy that logs all property access and modifications." },
        { title: "Memory Management", content: "Understanding memory allocation, garbage collection, and common memory leak patterns helps write efficient applications.", keyPoints: ["Stack vs Heap allocation", "Garbage collection basics", "WeakMap/WeakSet for efficiency", "Avoiding memory leaks"], codeExample: "// WeakMap for caching without leaks\nconst cache = new WeakMap();\n\nfunction processWithCache(obj) {\n  if (cache.has(obj)) return cache.get(obj);\n  const result = expensiveOperation(obj);\n  cache.set(obj, result);\n  return result;\n}", exercise: "Profile an app and identify memory leaks using Chrome DevTools." }
      ]
    },
    // Python Advanced Course
    {
      ...
      description: "Advanced Python programming mastery",
      slides: [
        { title: "Decorators and Metaprogramming", content: "Decorators modify function or class behavior and are essential for cross-cutting concerns like logging and caching.", keyPoints: ["Functions are first-class objects", "@decorator syntax", "functools.wraps preserves metadata", "Class decorators available"], codeExample: "import functools\n\ndef timing(func):\n    @functools.wraps(func)\n    def wrapper(*args, **kwargs):\n        import time\n        start = time.time()\n        result = func(*args, **kwargs)\n        print(f'{func.__name__}: {time.time()-start:.4f}s')\n        return result\n    return wrapper\n\n@timing\ndef slow_function():\n    pass", exercise: "Create decorators for caching and rate limiting." },
        { title: "Context Managers", content: "Context managers ensure proper resource handling with automatic cleanup using the with statement.", keyPoints: ["__enter__ and __exit__ methods", "contextlib for generators", "Automatic cleanup on errors", "Nested context managers"], codeExample: "from contextlib import contextmanager\n\n@contextmanager\ndef timer():\n    import time\n    start = time.time()\n    yield\n    print(f'Elapsed: {time.time()-start:.4f}s')\n\nwith timer():\n    # Do work\n    pass", exercise: "Create a context manager for database transactions with rollback." },
        { title: "Async/Await Concurrency", content: "Python's asyncio enables concurrent I/O-bound programming for building scalable applications.", keyPoints: ["Event loop manages coroutines", "async def for coroutines", "await for suspension points", "asyncio.gather for parallelism"], codeExample: "import asyncio\n\nasync def fetch_data(url):\n    # Simulated async fetch\n    await asyncio.sleep(1)\n    return f'Data from {url}'\n\nasync def main():\n    results = await asyncio.gather(\n        fetch_data('url1'),\n        fetch_data('url2')\n    )\n    print(results)\n\nasyncio.run(main())", exercise: "Build an async web scraper that fetches multiple pages concurrently." },
        { title: "Type Hints and Generics", content: "Type hints enable better IDE support and static analysis for more maintainable code.", keyPoints: ["Optional type annotations", "Generic types with TypeVar", "Protocol for structural typing", "mypy for static checking"], codeExample: "from typing import List, Optional, TypeVar, Generic\n\nT = TypeVar('T')\n\nclass Stack(Generic[T]):\n    def __init__(self) -> None:\n        self._items: List[T] = []\n    \n    def push(self, item: T) -> None:\n        self._items.append(item)\n    \n    def pop(self) -> Optional[T]:\n        return self._items.pop() if self._items else None", exercise: "Add comprehensive type hints to an existing Python project." }
      ]
    },
    // React Advanced Course
    {
      ...
      slides: [
        { title: "Custom Hooks", content: "Custom hooks extract and reuse stateful logic across components, enabling clean code organization.", keyPoints: ["Must start with 'use' prefix", "Can use other hooks", "Share logic without render props", "Compose for complex functionality"], codeExample: "function useFetch<T>(url: string) {\n  const [data, setData] = useState<T | null>(null);\n  const [loading, setLoading] = useState(true);\n  \n  useEffect(() => {\n    fetch(url)\n      .then(res => res.json())\n      .then(setData)\n      .finally(() => setLoading(false));\n  }, [url]);\n  \n  return { data, loading };\n}", exercise: "Create custom hooks for form handling, infinite scroll, and debounced search." },
        { title: "Performance Optimization", content: "Optimize React apps by preventing unnecessary re-renders and memoizing expensive computations.", keyPoints: ["React.memo for components", "useMemo for calculations", "useCallback for functions", "React.lazy for code splitting"], codeExample: "const ExpensiveList = memo(({ items, onItemClick }) => (\n  <ul>\n    {items.map(item => (\n      <li key={item.id} onClick={() => onItemClick(item.id)}>\n        {item.name}\n      </li>\n    ))}\n  </ul>\n));\n\nfunction Parent({ data }) {\n  const filtered = useMemo(() => \n    data.filter(x => x.active), [data]);\n  const handleClick = useCallback((id) => \n    console.log(id), []);\n  return <ExpensiveList items={filtered} onItemClick={handleClick} />;\n}", exercise: "Profile a React app and optimize performance bottlenecks." },
        { title: "Context and State Management", content: "React Context with useReducer provides Redux-like state management for complex applications.", keyPoints: ["Context for global state", "useReducer for complex logic", "Multiple contexts for separation", "Context selectors for optimization"], codeExample: "const CartContext = createContext(null);\n\nfunction cartReducer(state, action) {\n  switch (action.type) {\n    case 'ADD': return {...state, items: [...state.items, action.payload]};\n    case 'REMOVE': return {...state, items: state.items.filter(i => i.id !== action.payload)};\n    default: return state;\n  }\n}\n\nfunction CartProvider({ children }) {\n  const [state, dispatch] = useReducer(cartReducer, { items: [] });\n  return <CartContext.Provider value={{state, dispatch}}>{children}</CartContext.Provider>;\n}", exercise: "Build a state management solution with multiple contexts and undo/redo." },
        { title: "Error Boundaries and Suspense", content: "Error boundaries catch JavaScript errors while Suspense handles async loading gracefully.", keyPoints: ["Error boundaries are class components", "Catch rendering errors", "Suspense for lazy loading", "Fallback UI for loading/error"], codeExample: "class ErrorBoundary extends Component {\n  state = { hasError: false };\n  \n  static getDerivedStateFromError(error) {\n    return { hasError: true };\n  }\n  \n  render() {\n    if (this.state.hasError) {\n      return <h2>Something went wrong</h2>;\n    }\n    return this.props.children;\n  }\n}\n\n// Usage\n<ErrorBoundary>\n  <Suspense fallback={<Loading />}>\n    <LazyComponent />\n  </Suspense>\n</ErrorBoundary>", exercise: "Implement comprehensive error handling with retry logic and error reporting." }
      ]
    },
